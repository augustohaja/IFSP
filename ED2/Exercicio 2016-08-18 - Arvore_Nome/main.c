...
/*Arv* arv_cria (char letra, Arv* sae, Arv* sad){
    printf("\nDebug arv_cria");
...
/*int arv_pertence (Arv* arvAtual, char letra){
    if (arv_vazia(arvAtual) != 1){
        return 0;
    }
    else {
        if (strcmp(arvAtual->info, letra) == 1){
            return 1;
        }
        else {
            if (strcmp(letra,arvAtual->info) < 0){
                return arv_pertence(arvAtual->esq,letra);
            } else {
                return arv_pertence(arvAtual->dir,letra);
            }
        }
    }
}*/
